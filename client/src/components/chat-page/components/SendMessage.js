...
function SendMessage({ scroll, usuarioActual, socket }) {
    const [msg, setMsg] = useState('')

    const sendMessage = (ev) => {
        ev.preventDefault();
        const { _id, imgSrc, displayName} = usuarioActual;
        ...
        <>
            <form onSubmit={(ev)=>sendMessage(ev)}>
                <SendMessageContainer>
                    <Input placeholder='Mensaje...' type="text" value={msg} onChange={(ev) => setMsg(ev.target.value)} />
